<template>
  <section class="mt-32">
    <h2 class="text-4xl text-gray-500 font-bold">
      <span class="text-primary">Customize</span> your banner.
    </h2>
    <div class="flex mt-10">
      <div class="flex-1 flex justify-center items-center card mr-2">
        <img class="mr-2" src="../static/template.svg" alt="Template Icon" />
        <p class="font-bold text-gray-500">Template</p>
      </div>
      <div class="flex-1 flex justify-center items-center card mr-2">
        <img class="mr-2" src="../static/colors.svg" alt="Colors Icon" />
        <p class="font-bold text-gray-500">Colors</p>
      </div>
      <div class="flex-1 flex justify-center items-center card">
        <img class="mr-2" src="../static/follower-range.svg" alt="Follower Range Icon" />
        <p class="font-bold text-gray-500">Follower range</p>
      </div>
    </div>
  </section>
</template>

<script>
/*
<section class="mt-36 flex flex-col">
    <h2 class="text-5xl text-gray-500 font-bold">
      Choose <span class="text-primary">your colors</span>.
    </h2>
    <div class="mt-5 max-w-full min-h-full overflow-hidden">
      <div class="flex opacity-80">
        <img class="border rounded-sm mr-2" src="../assets/examples/green.png" alt="Green banner" />
      </div>
    </div>
    <h2 class="text-5xl text-gray-500 font-bold">
      Choose <span class="text-primary">your colors</span>.
    </h2>
    <div class="mt-5 max-w-full min-h-full overflow-hidden">
      <div class="flex carousel opacity-80">
        <img class="border rounded-sm mr-2" src="../assets/examples/green.png" alt="Green banner" />
        <img
          class="border rounded-sm mr-2"
          src="../assets/examples/orange.png"
          alt="Orange banner"
        />
        <img class="border rounded-sm" src="../assets/examples/purple.png" alt="Purple banner" />
      </div>
    </div>
    <h2 class="text-5xl text-gray-500 font-bold">
      Select <span class="text-primary">your pace</span>.
    </h2>
  </section>
*/
</script>
